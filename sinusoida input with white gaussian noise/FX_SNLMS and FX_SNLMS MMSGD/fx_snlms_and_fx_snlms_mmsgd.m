...
for k=1:length(t)                       
   ...
   Cw=Cw+mu*sign(e_cont(k))/sqrt(Xhx*Xhx'+ 1e-3)*Xhx; 
end
   err(itr,:)=e_cont.^2;
end
m=mean(err);
...
for k=1:length(t)                       
    ...
    Xhx=[S_temp Xhx(1:end-1)];
   g=g*c;
    if dot(vw,-1*(sign(e_cont(k))/sqrt(Xhx*Xhx'+ 1e-3)*Xhx))<0
        vw=-1*mu*(sign(e_cont(k))/sqrt(Xhx*Xhx'+ 1e-3)*Xhx);
    else 
       vw=(-1*mu*(sign(e_cont(k))/sqrt(Xhx*Xhx'+ 1e-3)*Xhx)+ (g*vw));
    end
    Cw=Cw-vw;
...
ylabel('MSE (dB)');
xlabel('iteration');
title('FX-SNLMS VS FX-SNLMS MMSGD')
legend('FX-SNLMS','FX-SNLMS MMSGD');
grid on
toc;
