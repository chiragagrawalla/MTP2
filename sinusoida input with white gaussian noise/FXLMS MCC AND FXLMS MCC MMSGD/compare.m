...
for k=1:length(t)                       
   ...
   Cw=Cw+mu*exp(-e_cont(k)^2/(2*sgm^2))*e_cont(k)*Xhx; 
end
   err(itr,:)=e_cont.^2;
end
m=mean(err);
...
for k=1:length(t)                       
    ...
    Xhx=[S_temp Xhx(1:end-1)];
   g=g*c;
    if dot(vw,-1*exp(-e_cont(k)^2/(2*sgm^2))*e_cont(k)*Xhx)<0
        vw= -1*mu*exp(-e_cont(k)^2/(2*sgm^2))*e_cont(k)*Xhx;
    else 
       vw= (-1*mu*exp(-e_cont(k)^2/(2*sgm^2))*e_cont(k)*Xhx)+ (g*vw);
    end
    Cw=Cw-vw;
...
ylabel('MSE (dB)');
xlabel('iteration');
title(' FxLMS MCC vs FxLMS MCC MMSGD')
legend('FXLMS MCC','FXLMS MCC MMSGD');
grid on
toc;
