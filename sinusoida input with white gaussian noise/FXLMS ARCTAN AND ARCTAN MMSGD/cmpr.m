...
for k=1:length(t)                       
   ...
   Cw=Cw+mu*(norm(Xhx)^2*lam*e_cont(k)/(norm(Xhx)^4+((lam^2)*e_cont(k)^4)))*Xhx; 
end
   err(itr,:)=e_cont.^2;
end
m=mean(err);
figure
plot(10*log10(smooth(m,100)),'k-.');
xlim([0 10000]);
...
for k=1:length(t)                       
    ...
    Xhx=[S_temp Xhx(1:end-1)];
   g=g*c;
    if dot(vw,-1*(norm(Xhx)^2*lam*e_cont(k)/(norm(Xhx)^4+((lam^2)*e_cont(k)^4)))*Xhx)<0
        vw=-1*mu*(norm(Xhx)^2*lam*e_cont(k)/(norm(Xhx)^4+((lam^2)*e_cont(k)^4)))*Xhx;
    else 
       vw=(-1*mu*(norm(Xhx)^2*lam*e_cont(k)/(norm(Xhx)^4+((lam^2)*e_cont(k)^4)))*Xhx)+ (g*vw);
    end
    Cw=Cw-vw;
...
ylabel('MSE (dB)');
xlabel('iteration');
title('Comparision of MSE for FXLMS ARCTAN and FXLMS ARC MMSGD algorithm');
legend('FXLMS ARCTAN','FXLMS ARCTAN MMSGD');
grid on
toc;
