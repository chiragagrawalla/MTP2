...
for k=1:length(t)                       
   ...
   %Cw=Cw+mu*norm(Cx)^2*lam*e_cont(k)/(norm(Cx)^4+((lam^2)*e_cont(k)^4))*e_cont(k)*Xhx; 
   %Cw=Cw+mu*((log(abs(e_cont(k))))/(abs(e_cont(k))*abs(e_cont(k))'))*e_cont(k)*Xhx;
   Cw=Cw+(mu*e_cont(k)*Xhx/(sgm^2+e_cont(k)^2));
end
   err(itr,:)=e_cont.^2;
end
m=mean(err);
...
for k=1:length(t)                       
    ...
    Xhx=[S_temp Xhx(1:end-1)];
    g=g*c;
    if dot(vw,-1*e_cont(k)*Xhx/(sgm^2+e_cont(k)^2))<0
        vw=-1*mu*e_cont(k)*Xhx/(sgm^2+e_cont(k)^2);
    else 
       vw=-1*mu*e_cont(k)*Xhx/(sgm^2+e_cont(k)^2)+(g*vw);
    end
    Cw=Cw-vw;
...
ylabel('MSE (dB)');
xlabel('iteration');
title('FxLMS LOG vs FxLMS LOG MMSGD')
legend('FXLMS Log','FXLMS Log MMSGD');
grid on
toc;
